...
async function loadStatuses() {
    const response = await fetch('/Manager/GetStatuses');
...
async function loadOrders(clientName = '', statusName = 'all') {
    const response = await fetch(`/Manager/GetOrders?clientName=${clientName}&statusName=${statusName}`);
...
function viewOrder(orderId) {
    window.location.href = `/Manager/ViewOrder/${orderId}`;
}
